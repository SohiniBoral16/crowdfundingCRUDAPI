...
public class PartyRelationshipVisualizationService {
    ...
    public PartyRelationshipVisualization getPartyDetails(Party party) {
        ...
        partyDetails.setPartyId(party.getPartyID());
        partyDetails.setPartyName(getFirstNonEmptyValue(
            () -> party.getPartyNameList() != null && !party.getPartyNameList().isEmpty() ? 
            party.getPartyNameList().get(0).getName() : null));
        partyDetails.setPartyAlias(party.getPartyAlias());

        // Using Supplier for getCountryOfDomicile logic
        partyDetails.setCountryOfDomicile(getCountryValue(() -> party.getCountryOfDomicile()));

        // Other attributes...
        partyDetails.setCountryOfOrganization(getFirstNonEmptyValue(
            () -> party.getCountryOfOrganization() != null ? party.getCountryOfOrganization().getName() : null));
        partyDetails.setDateOfBirth(getFirstNonEmptyValue(
            () -> party.getDateOfBirth() != null ? party.getDateOfBirth().toString() : null));
        partyDetails.setDateOfIncorporation(getFirstNonEmptyValue(
            () -> party.getFormationDate() != null ? party.getFormationDate().toString() : null));
        partyDetails.setLegalForm(getFirstNonEmptyValue(
            () -> party.getLegalForm() != null ? party.getLegalForm().getName() : null));
        partyDetails.setLegalName(getFirstNonEmptyValue(
            () -> party.getLegalForm() != null ? party.getLegalForm().getLegalName() : null));
        partyDetails.setValidationStatus(getFirstNonEmptyValue(
            () -> party.getPartyValidationStatus() != null ? party.getPartyValidationStatus().getName() : null));
        partyDetails.setPepIndicator(getPepIndicator(party));
        partyDetails.setCountrySpecificIdentifier(getCountrySpecificIdentifier(party));

        return partyDetails;
    }

    private <T> T getFirstNonEmptyValue(Supplier<T> supplier) {
        return Optional.ofNullable(supplier.get()).orElse(null);
    }

    private String getCountryValue(Supplier<Country> supplier) {
        return Optional.ofNullable(supplier.get())
                .map(country -> Optional.ofNullable(country.getName()).orElse(country.getId().toString()))
                .orElse(null);
    }

    // Other methods like getPepIndicator, getCountrySpecificIdentifier, etc...
}
